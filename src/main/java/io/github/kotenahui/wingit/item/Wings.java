...
public class Wings extends TemplateArmorItem implements ArmorTextureProvider {
    ...
    public String tryConsumeFeather(Entity entity) {
        ...
        int[] bagIndexes = WingIt.indexesOf(WingIt.item_FEATHER_BAG.id, player.inventory);
        ArrayList<FeatherBag> bags = new ArrayList<>();
        if (bagIndexes.length != 0) {
            /*for (int i = 0; i < player.inventory.main.length; i++) {
                if (player.inventory.main[i].itemId == WingIt.item_FEATHER_BAG.id) {
                    bags.add((FeatherBag)player.inventory.main[i].getItem());
                }
            }*/
            for (int i = 0; i < bagIndexes.length; i++) {
                if (player.inventory.main[bagIndexes[i]].itemId == WingIt.item_FEATHER_BAG.id) {
                    bags.add((FeatherBag)player.inventory.main[bagIndexes[i]].getItem());
                }
            }
        }
        if (entity.isSneaking()) { //Inventory feather
            if (bagIndexes.length != 0) {
                for (int i = 0; i < bagIndexes.length; i++) {
                    if (bags.get(i).GetState(player.inventory.main[bagIndexes[i]]).equals("feather")) {
                        player.inventory.main[bagIndexes[i]].damage(1, entity);
                        return "feather";
                    }
                }
            }
            if (player.inventory.remove(Item.FEATHER.id)) {
                return "feather";
            }
        }
        else if (!entity.isSneaking()) { //Inventory dense feather
            if (bagIndexes.length != 0) {
                for (int i = 0; i < bagIndexes.length; i++) {
                    if (bags.get(i).GetState(player.inventory.main[bagIndexes[i]]).equals("dense_feather")) {
                        player.inventory.main[bagIndexes[i]].damage(1, entity);
                        return "dense_feather";
                    }
                }
            }
            if (player.inventory.remove(WingIt.item_DENSE_FEATHER.id)) {
                return "dense_feather";
            }
        }
        return null;
